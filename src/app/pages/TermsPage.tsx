...
export function TermsPage() {
  const L = useL();
  const sections = [
    [L("Booking requests", "طلبات الحجز"), L("A submitted request is not confirmed until LuxRide accepts the journey details and availability directly with the customer.", "لا يُعد الطلب المرسل مؤكداً حتى تعتمد LuxRide تفاصيل الرحلة والتوفر مباشرةً مع العميل.")],
  ...
  return <PageShell crumb={L("Terms and Conditions", "الشروط والأحكام")} title={L("Terms and Conditions", "الشروط والأحكام")} subtitle={L("The main conditions for LuxRide transfer enquiries and booking requests.", "الشروط الأساسية لاستفسارات النقل وطلبات الحجز لدى LuxRide.")}><section className="bg-lux-beige py-16 md:py-24"><div className="mx-auto max-w-3xl space-y-5 px-4 md:px-8">{sections.map(([title, text]) => <article key={title} className="rounded-2xl border border-lux-charcoal/10 bg-white p-7"><h2 className="text-lux-charcoal" style={{ fontSize: "1.25rem", fontWeight: 700 }}>{title}</h2><p className="mt-3 text-sm text-neutral-600" style={{ lineHeight: 1.7 }}>{text}</p></article>)}</div></section></PageShell>;
}
