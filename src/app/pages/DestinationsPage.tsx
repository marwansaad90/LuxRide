...
type DestinationRoute = Route
  & Pick<CmsDestination, "displayFrom" | "displayTo" | "imagePosition">
  & Partial<Pick<CmsDestination, "fromPrice" | "oldPrice" | "discountPct">>;
...
function cmsDestinationRoute(destination: CmsDestination): DestinationRoute | null {
  ...
  return {
    ...
    fromPrice: Number(destination.fromPrice) > 0 ? Number(destination.fromPrice) : undefined,
    oldPrice: Number(destination.oldPrice) > 0 ? Number(destination.oldPrice) : undefined,
    discountPct: Number(destination.discountPct) > 0 ? Number(destination.discountPct) : undefined,
  };
}
            ...
            <div key={group.en}>
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {group.routes.map((destination) => {
                  ...
                  const salePrice = Number(routeItem.fromPrice ?? price) > 0 ? Number(routeItem.fromPrice ?? price) : undefined;
                  const oldPrice = Number(routeItem.oldPrice) > 0 ? Number(routeItem.oldPrice) : undefined;
                  const configuredDiscountPct = Number(routeItem.discountPct) > 0 ? Number(routeItem.discountPct) : undefined;
                  const hasValidDiscount = Boolean(salePrice && oldPrice && oldPrice > salePrice && configuredDiscountPct);
                  const discountPct = hasValidDiscount && oldPrice && salePrice
                    ...
                    <article key={routeItem.id} className="group overflow-hidden rounded-2xl bg-white shadow-[0_10px_40px_rgba(0,0,0,0.06)] transition-all hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)]">
                      <div className="relative h-44 overflow-hidden bg-white">
                        ...
                        <div className="absolute right-4 top-4 flex items-center gap-2">
                          {hasValidDiscount && <span className="rounded-full bg-lux-orange px-3 py-1 text-xs font-bold text-white shadow-sm">{L("Special Offer", "عرض خاص")}</span>}
                          <span className="rounded-full bg-lux-green px-3 py-1 text-xs text-white">{L("from", "من")} €{salePrice}</span>
                        </div>
                      </div>
                      <div className="p-6">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-neutral-500">
                          {hasValidDiscount && oldPrice && salePrice && (
                            <div className="flex items-baseline gap-2" aria-label={L("Sale price", "سعر العرض")}>
                              <span className="text-xs text-neutral-400 line-through">€{oldPrice}</span>
                              <span className="font-bold text-lux-green">{L("from", "من")} €{salePrice}</span>
                              {discountPct && discountPct > 0 && <span className="rounded-full bg-lux-orange/10 px-2 py-0.5 text-xs font-semibold text-lux-orange">-{discountPct}%</span>}
                            </div>
                          )}
                          ...
                          {showRecommendationBadge && <span className="rounded-full bg-lux-green/10 px-2 py-0.5 text-xs font-medium text-lux-green">{L("Popular: Round Trip", "شائع: ذهاب وعودة")}</span>}
                        </div>
