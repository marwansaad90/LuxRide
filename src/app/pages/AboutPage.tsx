...
export function AboutPage() {
  ...
  const cards = [
    ...
    { icon: CarFront, title: L("Fleet and Comfort", "الأسطول والراحة"), text: L("The modern fleet includes an MPV, sedan and mini van for client testing, all air-conditioned and matched to the passenger and luggage limits shown during booking.", "يشمل الأسطول الحديث سيارة MPV وسيدان وميني فان لاختبار العميل، وجميعها مكيفة وتطابق حدود الركاب والأمتعة المعروضة أثناء الحجز.") },
    ...
    { icon: Wallet, title: L("Transparent Pricing", "أسعار شفافة"), text: L("Workbook-derived fares are fixed in EUR and tax inclusive. Applicable airport or permit fees are listed separately before submission, with no hidden charges.", "الأسعار المستندة إلى ملف الأسعار ثابتة باليورو وشاملة الضريبة. تُدرج رسوم المطار أو التصريح المطبقة بشكل منفصل قبل الإرسال، من دون رسوم خفية.") },
      ...
      <section className="bg-lux-beige py-16 md:py-24" aria-labelledby="about-introduction">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-4 md:px-8 lg:grid-cols-2">
          <div className="overflow-hidden rounded-3xl border border-white/70 bg-white p-2 shadow-[0_20px_60px_rgba(15,22,35,0.10)]">
            <ImageWithFallback loading="lazy" src={IMAGES.hurghada} alt={L("LuxRide private transport in Hurghada", "نقل LuxRide الخاص في الغردقة")} className="h-[420px] w-full rounded-2xl object-cover" />
          </div>
          <div>
