  ...
  return (
    <PageShell crumb={L("Booking Submitted", "تم إرسال الحجز")} title={L("Your Request Has Been Submitted", "تم إرسال طلبك")} subtitle={L("Thank you. Your booking request has been received and will be reviewed before confirmation.", "شكراً لك. تم استلام طلب حجزك وستتم مراجعته قبل التأكيد.")}>
      <section className="bg-lux-beige py-16 md:py-24"><div className="mx-auto max-w-2xl px-4 md:px-8"><div className="rounded-3xl border border-lux-green/30 bg-white p-8 text-center">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row"><a href={whatsappLink("Hi LuxRide, I just submitted a booking request.")} target="_blank" rel="noreferrer" className="flex flex-1 items-center justify-center gap-2 rounded-full bg-lux-green py-3 text-sm text-white transition-all hover:brightness-110"><MessageCircle className="h-4 w-4" /> {L("Message us on WhatsApp", "راسلنا على واتساب")}</a><Link to="/" className="flex flex-1 items-center justify-center rounded-full border border-lux-charcoal/15 py-3 text-sm text-lux-charcoal">{L("Back to Home", "العودة للرئيسية")}</Link></div>
      </div></div></section>
    </PageShell>
  );
}
